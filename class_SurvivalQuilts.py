...
from class_UnderlyingModels import CoxPH, CoxPHRidge, Weibull, LogNormal, LogLogistic, RandomSurvForest, LassoModel, ElasticNetModel, SuperPC, SVMModel, GBMModel, DeepHitModel, ConditionalInferenceSurvForestModel
...
class SurvivalQuilts:
    def __init__(self, K=10, num_bo=50, num_outer=3, num_cv=10, step_ahead=5):
        ...
        self.step_ahead = step_ahead  # Step ahead calculation for robust selection
        self.num_bo = num_bo  # Bayesian Optimization iteration
        self.num_outer = num_outer  # Maximum number of Bayesian Optimization iterations
        self.num_cv = num_cv  # Number of cross-validation folds
        self.lmbda = 0.
        self.rho = 0.5
        self.SEED = 1234
        self.model_names = [
            'CoxPH', 'CoxPHRidge', 'Weibull', 'LogNormal', 'LogLogistic', 'RandomSurvForest',
            'Lasso', 'ElasticNet', 'SuperPC', 'SVM', 'GBM', 'DeepHit', 'ConditionalInferenceSurvForest'
        ]
    ...
    def train(self, X, T, Y):
        ...
        for cv_idx in range(self.num_cv):
            print(f'CV.. {cv_idx + 1}/{self.num_cv}')
        ...
        X_inits[0, np.argmax(np.mean(metric_CINDEX, axis=0)[:, 0])] = 1  # Put more weight on the "best" model at the first step
        ...
        for k in range(self.K):
            ...
            X_inits = np.zeros([1, self.M])
            X_inits[0, np.argmax(np.mean(metric_CINDEX, axis=0)[:, k])] = 1  # Put more weight on the "best" model at the first step
            X_inits = self._get_normalized_X_step(X_inits)
            ...
            for out_itr in range(self.num_outer):
                ...
                for itr in range(self.num_bo):
                    gp = GPyOpt.methods.BayesianOptimization(
                        f=None, domain=self.ens_domain, X=X_step_ens, Y=Y_step_ens,
                        acquisition_type='EI', model_type='GP', exact_feval=True, cost_withGradients=None
                    )
                ...
                if GP_ens is not None:
                    ...
                    print(f'out_itr: {out_itr} | BEST X: {X_opt}')
                    tmp_o, tmp_c = self._get_Y_step_pulled(W, X, T, Y, K_step=k)
                    ...
                    if max(0, tmp_c[0] - beta_) < 0.005 * beta_:  # 1% off from the median
                        ...
                        print(f'BEST: {X_opt}')
                        print(f'Objective val.: {tmp_o[0]}')
                        print(f'Constraint val.: {tmp_c[0]}')
                        print('====================================')
                        break
                else:
                ...
                print(f'out_itr: {out_itr} | Lambda: {lmbda_} | Rho: {rho_}')
    ...
    def _make_ModelList(self):
        ...
        for tmp_name in self.model_names:
            if tmp_name == 'CoxPH':
                ...
                models += [LassoModel()]
            elif tmp_name == 'ElasticNet':
                models += [ElasticNetModel()]
            elif tmp_name == 'SuperPC':
                models += [SuperPC()]
            elif tmp_name == 'SVM':
                models += [SVMModel()]
            elif tmp_name == 'GBM':
                models += [GBMModel()]
            elif tmp_name == 'DeepHit':
                models += [DeepHitModel()]
            elif tmp_name == 'ConditionalInferenceSurvForest':
                models += [ConditionalInferenceSurvForestModel()]
        return models
